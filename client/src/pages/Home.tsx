...
import {
  ...
  ArrowRight, Quote, Play, Pause, Zap,
  Package, Users, TrendingUp, TreePine, Gem, Dumbbell, Medal, Landmark, PenLine
} from "lucide-react";
        ...
        <div className="marquee-container">
          <div className="marquee-track">
            {[...MARQUEE_ITEMS, ...MARQUEE_ITEMS, ...MARQUEE_ITEMS].map((item, i) => (
              <span key={i} className="flex items-center gap-4 px-6 text-sm font-['Nunito_Sans'] font-600 tracking-wide text-muted-foreground whitespace-nowrap uppercase">
                <svg viewBox="0 0 12 12" className="w-2.5 h-2.5 text-gold flex-shrink-0" fill="currentColor">
                  <path d="M6 0L7.5 4.5H12L8.25 7.5L9.75 12L6 9L2.25 12L3.75 7.5L0 4.5H4.5Z"/>
                </svg>
                {item}
              </span>
            ))}
                    ...
                    <div className="bg-card rounded-[calc(var(--radius-lg)-1px)] overflow-hidden">
                      <div className="relative h-56 overflow-hidden bg-gradient-to-b from-accent/20 to-accent/50 flex items-center justify-center">
                        ...
                        <div className="absolute top-4 right-4 w-10 h-10 rounded-xl bg-gold/20 backdrop-blur-sm border border-gold/30 flex items-center justify-center">
                          {(() => { const Icon = CATEGORY_ICONS[cat.icon]; return Icon ? <Icon className="w-5 h-5 text-gold" /> : null; })()}
                        </div>
